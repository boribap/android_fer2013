...
float resize_img(Mat img_src, Mat &img_resize, int resize_width) {
   ...
   // __android_log_print(ANDROID_LOG_DEBUG, (char *) "native-lib :: ", (char *) "face_src_width %d ", (float)img_src.cols);
    //__android_log_print(ANDROID_LOG_DEBUG, (char *) "native-lib :: ", (char *) "face_src_scale %d ", scale);
...
Java_com_boribap_forkids_MainActivity_detect(JNIEnv *env, jobject instance,
    ...
    for(int i=0;i<faces.size();i++){
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "native-lib:: ", "facesize %d,%d", real_facesize_width,real_facesize_height);
...
//        ellipse(img_result, center, Size(real_facesize_width/2, real_facesize_height/2), 0, 0, 360, Scalar(255,0,255), 30, 8, 0);
        rectangle(img_result, Point(real_facesize_x,real_facesize_y), Point(real_facesize_x + real_facesize_width, real_facesize_y+real_facesize_height), Scalar(255,0,255), 15, 8, 0);


        putText( img_result, to_string(real_facesize_width), center, 2, 1.2, Scalar::all(255) );
